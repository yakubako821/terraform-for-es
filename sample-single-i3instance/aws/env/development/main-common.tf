...
terraform {
  ...
  backend "s3" {
    ...
    key    = "cpf-elasitcsearch-sample/single-not-i3instance/terraform.tfstate"
...
module "elasticsearch_route53_record_indexing" {
  source = "../../modules/route53_record"

  common_tags              = local.common_tags
  route53_record_parameter = local.route53_record_parameter_indexing
}
