...
data "template_file" "container_definition_elasticsearch-master_1" {
  ...
  vars = {
    ...
    cluster_initial_master_nodes                                            = local.elasticsearch_parameter_1["discovery_type_single"] ? "" : "-E cluster.initial_master_nodes=${local.elasticsearch_parameter_1["master_node_ips"][0]},${local.elasticsearch_parameter_1["master_node_ips"][1]}"
...
locals {
  elasticsearch_parameter_1 = {
    ...
    master_node_ips           = list("172.24.68.111", "172.24.76.111")#, "172.24.84.111")
    master_memory_reservation = 7168
    master_es_java_opts       = "-XX:ActiveProcessorCount=4 -Xms4096m -Xmx4096m -XX:HeapDumpPath=/usr/share/elasticsearch/data -Xlog:gc*:file=/usr/share/elasticsearch/logs/gc_%p_%t.log::filecount=32,filesize=64m:time"
...
locals {
  ...
  elb_listener_targetgroup_parameter_kibana_1 = {
    ...
    listener_certificate_arn                      = "arn:aws:acm:ap-northeast-1:${data.aws_caller_identity.current.account_id}:certificate/9b3082f7-9d43-4df5-806b-b19fcc56f5c4"
    target_type                                   = "instance"
