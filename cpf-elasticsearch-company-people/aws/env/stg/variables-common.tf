...
data "template_file" "container_definition_elasticsearch-datadog-agent" {
  template = file("../../../../cpf-elasticsearch-article-archive/aws/modules/definitions/container_definitions/container_definition_elasticsearch-datadog-agent.json")
...
variable "vpc_id" {
  default = "vpc-9fdd9ff8"
}

variable "private_subnet_ids" {
  type    = "list"
  default = ["subnet-08f4ff41", "subnet-f2dc99a9", "subnet-88ffe2a0"]
}

variable "public_subnet_ids" {
  type    = "list"
  default = ["subnet-b8f0fbf1", "subnet-3fd89d64", "subnet-4efee366"]
}

variable "privatesubnet_cidr_blocks" {
  type    = "list"
  default = ["172.24.68.0/23", "172.24.76.0/23", "172.24.84.0/23"]
}
...
locals {
  elasticsearch_name       = "${var.project_name}-elasticsearch-company-people"
  elasticsearch_short_name = "${var.project_name}-es-cp"
}
  ...
  security_group_parameter = {
    ...
    ingress = [
      ...
      },
      {
        ...
        security_groups = list("sg-0532476063e16af49")
        cidr_blocks     = null
        self            = null
      }
    ]
  ...
  security_group_parameter_elb-for-kibana = {
    ...
    ingress = [
      ...
      },
      {
        ...
        to_port         = 443
        protocol        = "tcp"
        description     = "from Minami-suna"
        security_groups = null
        cidr_blocks     = list("138.101.21.128/25")
        self            = null
      },
      {
        from_port       = 443
        to_port         = 443
        protocol        = "tcp"
        description     = "from Nikkei-Suzuki-VPN"
        security_groups = null
