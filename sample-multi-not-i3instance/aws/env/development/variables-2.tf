...
data "template_file" "container_definition_elasticsearch-master_2" {
  ...
  vars = {
    ...
    cluster_initial_master_nodes                                            = local.elasticsearch_parameter_2["discovery_type_single"] ? "" : "-E cluster.initial_master_nodes=${local.elasticsearch_parameter_2["master_node_ips"][0]},${local.elasticsearch_parameter_2["master_node_ips"][1]},${local.elasticsearch_parameter_2["master_node_ips"][2]}"
...
locals {
  elasticsearch_parameter_2 = {
    ...
    master_node_ips           = list("172.24.101.114", "172.24.109.114", "172.24.117.114")
    ...
    data_instance_type      = "i3.4xlarge"
    data_autoscaling_size   = 42
    data_memory_reservation = 100000
    data_es_java_opts       = "-XX:ActiveProcessorCount=16 -Xms32766m -Xmx32766m -XX:HeapDumpPath=/usr/share/elasticsearch/data -Xlog:gc*:file=/usr/share/elasticsearch/logs/gc_%p_%t.log::filecount=32,filesize=64m:time"

    kibana-and-coordinate_instance_type    = "r5.xlarge"
    kibana-and-coordinate_autoscaling_size = 3
    coordinate_memory_reservation          = 25600
    kibana_memory_reservation              = 2048
    coordinate_es_java_opts                = "-XX:ActiveProcessorCount=4 -Xms14336m -Xmx14336m -XX:HeapDumpPath=/usr/share/elasticsearch/data -Xlog:gc*:file=/usr/share/elasticsearch/logs/gc_%p_%t.log::filecount=32,filesize=64m:time"
